...
function mostrarPopup(mensagem) { mostrarPopupGlobal(mensagem || 'Ação concluída!'); }
function mostrarPopupMarcar(mensagem) { mostrarPopupGlobal(mensagem || 'Agendamento realizado com sucesso!'); }
function mostrarPopupEdicao(mensagem) { mostrarPopupGlobal(mensagem || 'Cadastro atualizado com sucesso!'); }
function mostrarPopupCodigo(mensagem) { mostrarPopupGlobal(mensagem || 'Código gerado!'); }
function mostrarPopupPACNA(mensagem) { mostrarPopupGlobal(mensagem || 'Paciente não encontrado!'); }
function mostrarPopupVFCS(mensagem) { mostrarPopupGlobal(mensagem || 'Horário da consulta já está ocupado!'); }
function mostrarPopupDATA(mensagem) { mostrarPopupGlobal(mensagem || 'Data inválida!'); }
...
function editarPaciente() {
  ...
  paciente.nascimento = document.getElementById('nascimentoEditar')?.value?.trim();
...
function agendarConsulta() {
  ...
  const exame = (exameRaw && exameRaw.toLowerCase() !== 'nenhum') ? exameRaw : 'N/A';
  ...
  if (dataMarcada < hoje) {
    mostrarPopupPassado('Quem disse que voce pode voltar no passado? (arruma essa data)');
    return;
  }
  ...
  if (agendamentos.some(a => a.dataConsulta === dataConsulta && a.horaConsulta === horaConsulta && !a.cancelado)) {
  ...
  if (exame !== 'N/A' && dataExame) {
    ...
    if (agendamentos.some(a => a.dataExame === dataExame && a.horaExame === horaExame && !a.cancelado)) {
  ...
  // salva
  agendamentos.push({ codigo, consulta, dataConsulta, horaConsulta, exame, dataExame: dataExame || '-', horaExame: horaExame || '-' });
...
function verHistorico() {
  ...
  let html = '<ul>' + pacienteAgendamentos.map(a => `<li>Consulta: ${a.consulta} em ${a.dataConsulta} às ${a.horaConsulta}, Exame: ${a.exame} ${a.cancelado ? '<strong style="color:red"> — CANCELADA</strong>' : ''}</li>`).join('') + '</ul>';
    ...
    return `
      <div class="card-agendamento">
        <div class="card-row">
          ...
          <button class="btn small" style="float:right" onclick="cancelarAgendamentoDirect('${a.codigo}','${a.dataConsulta}','${a.horaConsulta}')">Cancelar</button>
        </div>
        <div><strong>Consulta:</strong> ${a.consulta} — ${a.dataConsulta} às ${a.horaConsulta}</div>
        <div><strong>Exame:</strong> ${exameTexto}${a.dataExame && a.dataExame !== '-' ? ` — ${a.dataExame} às ${a.horaExame || '-'}` : ''}</div>
      </div>
    `;
  }).join('');
...
function buscarPaciente() {
  ...
  mostrarAlerta('alertaEdicao', 'Dados carregados! Pode editar agora.', 'sucesso');
}